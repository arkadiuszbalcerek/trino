        ...
        implements Module
{
            ...
            AccessControlRulesRestExtractor rulesRestExtractor)
    {
        if (config.getRefreshPeriod() != null) {
            return ForwardingConnectorAccessControl.of(memoizeWithExpiration(
                    () -> {
                        log.info("Refreshing access control for catalog '%s' from: %s", catalogName, config.getRestUrl());
                        return create(catalogName, rulesRestExtractor);
                    },
    ...
    private RulesBasedAccessControl create(CatalogName catalogName, AccessControlRulesRestExtractor rulesRestExtractor)
    {
        AccessControlRules controlRules = rulesRestExtractor.extract(AccessControlRules.class);
